...
const GraphDataTable: React.FunctionComponent<GraphDataTableProps & RouteComponentProps<any>> = ({
  ...
  // {
  //   console.log(parsed)
  //   setfilterQueryParam([...filterQueryParam, { columnName: parsed.c, filterName: parsed.f, inputName: parsed.i }])
  //   console.log("this is filtered" + JSON.stringify(filterQueryParam))
  // }
  ...
  const getBoardDataAsQuery = (error: string) => {
    ...
    if (parsed.c) {
      ...
      cvalue = parsed.c as string

      const skip = checkForPagination();

      // console.log("Value of parsed s" + parsed.s + "1" + " value of parsed f " + parsed.f + "2" + " value of parsed c " + parsed.c + "3" + " value of parsed i" + parsed.i + "4")
  ...
  const addurl = () => {
    console.log("hello");
    let allFilters: Allfilters[] = Utility.getAllFilters(parsed.f, parsed.c, parsed.i);
    ...
    getCombinedFilterData(listOfattributes, selectedEntity, allFilters)
    return (window.location.href = `${urlLabels.BASE_URL
      }uri=${URI}&e=${selectedEntity}&th=${theme}&filterObj=${s.slice(1, s.indexOf("}") + 1)}`);
